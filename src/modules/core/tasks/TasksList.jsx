      ...
      .map((item)=> (
        {
          ...
          paragraphs: [
            `Module: ${item.module}`,
            `Default trigger: ${item.trigger.type == 'interval' ? 'Every ' + item.trigger.value + ' seconds' : 'Unknown or not set' }`,
            Object.keys(item.extra_typed).length > 0 ? (`Extra Params: ${Object.keys(item.extra_typed)}`) : '', 
